<template>
  <div class="space-y-6">
    ...
    <div
      v-if="viaje.participantes.length > 0 && viaje.gastos.length > 0"
      class="card"
    >
    ...
    <div v-else-if="viaje.gastos.length === 0" class="card text-center py-8">
      <div class="text-gray-500">
        ...
        <p class="text-sm">
          Ve a la pestaña "Gastos" para comenzar a registrar gastos del viaje
        </p>
      </div>
    </div>
  </div>
...
const datosGrafico = computed(() => {
  ...
  const gastosData = props.resumen.balances.map(
    (balance) => balance.totalPagado
  );
  const deudasData = props.resumen.balances.map(
    (balance) => balance.totalDeuda
  );
