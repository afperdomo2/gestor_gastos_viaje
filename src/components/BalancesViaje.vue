<template>
  <div class="space-y-6">
        ...
        >
          <div class="flex items-center space-x-3">
            ...
            <div>
              ...
              <div class="text-sm text-gray-600">
                Pagó: ${{ formatearMoneda(balance.totalPagado) }} • Debe: ${{
                  formatearMoneda(balance.totalDeuda)
                }}
              </div>
            </div>
          </div>
          <div class="text-right">
            <div
              ...
              :class="getBalanceTextClass(balance.balance)"
            >
              {{ balance.balance >= 0 ? "+" : "" }}${{
    ...
    <!-- Matriz de Deudas -->
    <div
      v-if="resumen && resumen.deudaMatriz && viaje.participantes.length > 1"
      class="card"
    >
      <h3 class="text-lg font-semibold text-gray-900 mb-4">Matriz de Deudas</h3>
      <p class="text-sm text-gray-600 mb-4">
        Las filas representan quien pagó, las columnas quien debe. Solo se
        muestran deudas mayores a $0.
      </p>
            ...
            <tr v-for="pagador in viaje.participantes" :key="pagador.id">
              ...
              >
                <span
                  ...
                  :class="
                    getDeudaClass(resumen.deudaMatriz[pagador.id][deudor.id])
                  "
                >
                  {{
    ...
    <div v-if="resumen && resumen.balances.length > 0" class="card">
      ...
      <div class="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div class="text-sm text-blue-700">
          ...
          <p class="font-medium">
            ⚖️ Diferencia: ${{
              formatearMoneda(Math.abs(totalAcreedores - totalDeudores))
            }}
            <span
              v-if="Math.abs(totalAcreedores - totalDeudores) < 0.01"
    ...
    <div
      v-if="!resumen || resumen.balances.length === 0"
      class="card text-center py-8"
    >
      <div class="text-gray-500">
        <p class="text-lg mb-2">⚖️ No hay balances para mostrar</p>
        <p class="text-sm">
          Registra algunos gastos para ver los balances entre participantes
        </p>
      </div>
    </div>
  </div>
...
const totalAcreedores = computed(() => {
  return acreedores.value.reduce(
...
const totalDeudores = computed(() => {
  return Math.abs(
    deudores.value.reduce((total, balance) => total + balance.balance, 0)
  );
});
