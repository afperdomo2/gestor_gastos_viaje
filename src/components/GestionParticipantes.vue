<template>
  <div class="space-y-6">
    ...
    <div class="card">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">Agregar Participante</h3>
    ...
    <div class="card">
      ...
      <div v-if="viaje.participantes.length === 0" class="text-center py-8 text-gray-500">
        ...
        <p class="text-sm">Agrega al menos una persona para comenzar a registrar gastos</p>
      </div>
      ...
      <div v-else class="space-y-3">
        ...
        >
          ...
          <button 
            @click="eliminarParticipanteConfirm(participante.id, participante.nombre)"
        ...
        <div class="ml-3">
          ...
          <div class="mt-2 text-sm text-blue-700">
            <ul class="list-disc list-inside space-y-1">
              <li>Puedes agregar o eliminar participantes en cualquier momento</li>
              <li>No se puede eliminar un participante que tenga gastos asociados</li>
              <li>Los nombres de los participantes aparecerán en los formularios de gastos</li>
            </ul>
          </div>
        </div>
      </div>
...
const agregarNuevoParticipante = (): void => {
  if (nombreParticipante.value.trim()) {
    ...
    const nombreExiste = props.viaje.participantes.some(
      p => p.nombre.toLowerCase().trim() === nombreParticipante.value.toLowerCase().trim()
    );
    ...
    const participante = agregarParticipante(props.viaje.id, nombreParticipante.value);
    if (participante) {
...
const eliminarParticipanteConfirm = (id: string, nombre: string): void => {
  if (confirm(`¿Estás seguro de que quieres eliminar a "${nombre}" del viaje? Esta acción no se puede deshacer.`)) {
    try {
...
const obtenerEstadisticasParticipante = (participanteId: string): string => {
  ...
  const gastosComoPagador = props.viaje.gastos.filter(gasto => 
  ...
  if (gastosComoPagador > 0) {
    partes.push(`${gastosComoPagador} pago${gastosComoPagador !== 1 ? 's' : ''}`);
  }
  if (gastosComoDeudor > 0) {
    partes.push(`${gastosComoDeudor} deuda${gastosComoDeudor !== 1 ? 's' : ''}`);
  }
